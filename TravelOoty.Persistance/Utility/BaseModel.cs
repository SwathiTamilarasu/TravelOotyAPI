...
namespace TravelOoty.Persistance.Utility
{
    public abstract class BaseModel
    {
        ...
        public static Func<T, bool> TranslateFilter<T>(string filter)
        {
            ...
            // var lookupMethod = modelType.GetMethod("TestMethod", BindingFlags.Public | BindingFlags.Static | BindingFlags.FlattenHierarchy);
            //  var lookupMethod = modelType.GetMethod("AddText", BindingFlags.Instance | BindingFlags.NonPublic, null, new Type[] { typeof(string) }, null);
            var modelProp = modelType.GetProperties().FirstOrDefault(prop => prop.Name.ToLower() == dataField.ToLower());

            if (modelProp == null)
            {
                modelProp = modelType.GetProperties().FirstOrDefault(prop => prop.Name == PropertyMap[dataField.ToLower()]);
            }
            try
            {
            ...
            {
                throw new Exception(dataFieldRaw);
            }
        }
